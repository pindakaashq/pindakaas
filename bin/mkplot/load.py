...
def load_json(stat_arr, options):
    ...
    for stat_obj in stat_arr:
        ...
        last_val = -1
        # print(f"{stat_obj.data=}", file=sys.stderr)
        for inst in stat_obj.insts_own:  # insts_own are sorted
            ...
            if stat_obj.data[inst]['status'] == True:
                ...
                if options['timeout'] is not None and val >= float(options['timeout']):
                    val = float(options['timeout'])
                elif val <= min_val:
                        val = min_val
                ...
                val = float(options['timeout'])
                if options['plot_type'] in {'cactus', 'instance'}:
                    val *= 10
        ...
        linestyle = stat_obj.preamble["linestyle"]

        label = label.strip()

        data.append((label, vals, num_solved, last_val, linestyle))
    ...
    def sort_key(x):
        divider = sum([ x[1] for x in x[1]])
        if divider == 0:
            return 10000
        else:
            return x[2] + len(x[1]) / divider

    # print(f"{data=}", file=sys.stderr)
    return sorted(data, key=sort_key, reverse=not options['reverse'])
    # return sorted(data, key=lambda x: x[2] + len(x[1][1]) / sum(x[1][1]), reverse=not options['reverse'])
            ...
            if tools != 'all':
                ...
                for vlist in stats:
                    ...
                    if val < float(options['timeout']):
                        ...
                        val = options['timeout']
                        if options['plot_type'] == {'cactus', 'instance'}:
                            val *= 10
