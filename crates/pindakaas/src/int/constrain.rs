		...
		/*
			...
			return match (x, y, z) {
				...
				| (IntVarEnc::Ord(y_ord), IntVarEnc::Bin(x_bin), _) => {
					...
					let y_bin =
						IntVarBin::from_bounds(db, y.lb(), y.ub(), format!("{}{cmp}y:B", y.label()));
...
pub mod tests {
	...
	use crate::{
		...
		int::{enc::GROUND_BINARY_AT_LB, ord::OrdEnc, required_lits, Dom, IntVarOrd},
	};
	...
	fn get_ord_x<DB: ClauseDatabase, C: Coefficient>(
		...
		let x = OrdEnc::new(
			db,
			&Dom::from_slice(&dom.iter(..).map(|x| x.end - C::one()).collect_vec()),
		);
		if consistent {
	...
	*/
	...
	fn from_dom<DB: ClauseDatabase, C: Coefficient>(
		...
		if dom.len() == 1 {
			...
			match enc {
				IntVarEncoding::Ord => OrdEnc::new(db, &Dom::from_slice(dom)).into(),
				IntVarEncoding::Bin => {
