...
impl<'a, Lit: Literal, PC: PositiveCoefficient> Encoder for TotalizerEncoder<'a, Lit, PC> {
	...
	fn encode<DB: ClauseDatabase<Lit = Lit>>(&mut self, db: &mut DB) -> Result<Self::Ret> {
		...
		let mut layer: Vec<HashMap<PC, Lit>> = self
			.lin
			.terms
			.iter()
			.map(|part| {
			...
			layer = layer
				...
				.map(|children| {
					...
					for c in left.iter().chain(right.iter()) {
						let w = std::cmp::min(*c.0, self.lin.k + PC::one()); // not capped in literature, but should be slightly better
					...
					for l in left {
						for r in right {
							let w = std::cmp::min(*l.0 + *r.0, self.lin.k + PC::one());
							let p = parent.entry(w).or_insert_with(|| db.new_var());
		...
		let root_gt_k = layer[0].get(&(self.lin.k+PC::one()))
